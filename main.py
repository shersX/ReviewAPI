...
"审查结果": [{
	...
	"评估结果": "符合",
	"理由": "全文共30页，结构完整，包含课题名称、申请人信息、立项依据、研究内容等部分，篇幅符合国自然青年项目常规要求。"
	},
	{
	"规则内容（需带规则序号）": "2. 全文格式是否规范（字体行距统一/标题序号匹配/空格检查）",
	"评估结果": "符合",
	"理由": "标题层级清晰，序号使用规范，未发现字体行距不统一或多余空格问题。"
	},
	…………
	{
	"规则内容（需带规则序号）": "49. 项目经费预算中，参照国自然同类项目，申请人填写的是否合理",
	"评估结果": "符合",
	"理由": "预算中设备费、业务费、劳务费分配合理，符合国自然要求。"
	}
	],
"统计": {
	"态度类不合格数量": "2条（11、22）",
	"能力类不合格数量": "7条（21、23、24、25、29、46、47）",
	"是否触发第14条一票否决": "False"
	}
}
'''
...
def build_audit_prompt(pdf_text: str) -> str:
    ...
    return f"""
你你是一个形式审查员。请根据以下49条规则，对这份PDF内容进行逐条审查。
...
- "评估结果"必须为：`符合`、`不符合`；
- 如为 `不符合`，必须填写简要理由。
###直接输出json形式结果，例子结构如下所示，不允许输出偏差和多余的内容（多余的内容指的是以```json开头和```）：
{outjson}
